...
class SM_PlaceCells(Neurons):
    """The SM_PlaceCells class defines a population of SM_PlaceCells. This class is a subclass of Neurons() and inherits it properties/plotting functions.

       Must be initialised with an Agent and a 'params' dictionary.

       SM_PlaceCells defines a set of 'n' place cells scattered across the environment. The firing rate is a functions of the distance from the Agent to the place cell centres. This function (params['description'])can be:
           • gaussian (default)
           • gaussian_threshold
           • diff_of_gaussians
           • top_hat
    ...
    def __init__(self, Agent, params={}):
        """Initialise SM_PlaceCells(), takes as input a parameter dictionary. Any values not provided by the params dictionary are taken from a default dictionary below.
        ...
        if ratinabox.verbose is True:
            print(
                "SM_PlaceCells successfully initialised. You can see where they are centred at using SM_PlaceCells.plot_place_cell_locations()"
            )
        return
    ...
    def get_state(self, evaluate_at="agent", **kwargs):
        ...
        if self.description == "diff_of_softmax":
            ratio = 1.5  # ratio of inhibitory to excitatory width
            
            # Calculate logits (negative squared distances)
            logits_exc = -(dist**2) / (2 * (widths**2))
            logits_inh = -(dist**2) / (2 * ((ratio * widths) ** 2))

            # numerical stability helper for softmax over the population (axis 0)
            def softmax(logits):
                shift = logits - np.max(logits, axis=0, keepdims=True)
                exps = np.exp(shift)
                return exps / np.sum(exps, axis=0, keepdims=True)

            firingrate = softmax(logits_exc) - softmax(logits_inh)
